    ...
    for v_x_small in range(int(min(np.ceil(v_x/float(v_notx)),v_x)),30+2*max(2,v_x+1)):
        ...
        try:  
            for r in blocks_list:
                ...
                if v_x_small>10:
                    print(blocks.shape,"t",extract_basis(A,field).shape[1],extract_basis(A,field).shape[1] *5 / blocks.shape[0]  )          
                try:
                    t=time()
                    U0_temp=cshrunk.wongblockpseudo(X,field=field)
                    t1=time()
                    '''
                    U0p=extract_basis(cshrunk.wongblock(X)[:v_x],field)
                    print(U0_temp.shape,U0p.shape,"e")
                    t2=time()
                    if t1-t>100:
                        print("new", np.round(t1-t,2)," s, old",np.round(t2-t1,2)," s")
                    assert(intersection(U0_temp,U0p,field).shape[1]==max(U0_temp.shape[1],U0p.shape[1]))
                    '''
                    ...
                    if str(e) =="nc-rk A<nc-rk B":
                        ...
                        raise 
                if not( U0 is None or intersection(U0,U0_temp,field).shape[1]==max(U0.shape[1] ,U0_temp.shape[1]) ) :
                    
                    #c_plus
                    span_rep0_temp= spanning_subrep(V, x, U0_temp)
                    subrep0_temp= subrep(V,span_rep0_temp)
                    ...
                    #disc_plus
                    v_notx_s_plus= ceil_div(v_notx*v_x_small,v_x)
                    ...
                    disc_plus = v_x_disc_plus/float(v_x_small) 
                     ...
                     #c_moins
                    span_rep0= spanning_subrep(V, x, U0)
                    subrep0= subrep(V,span_rep0)
                    ...
                    for ux in range(U0.shape[1]+1,U0_temp.shape[1]):
                        ...
                        u_notx_min_plus = ceil_div((v_x-v_x_disc_plus+v_x*ux*(v_notx_s_plus)),v_x_small*v_x)
                        assert( v_x_disc_plus//v_x> 
                                v_notx_s_plus*ux- v_x_small*u_notx_min_plus and 
                                 v_x_disc_plus//v_x <= 
                                    v_notx_s_plus*ux- v_x_small*(u_notx_min_plus-1))
                        u_notx_min_minus = ceil_div((-v_x_disc_minus+v_x*ux*(v_notx_s_minus)),v_x*v_x_small)
    ...
    while len(file)>0:
        for e in V.edges_out[file.pop()]:
            if  V.edges[e][1]  in maps_span.keys():
                #check commutativity
                try:
                    assert(is_all_zero_mat(maps_span[V.edges[e][1]]-np.dot( V.Ve[e], maps_span[V.edges[e][0]]), field))
                except:
                    #V.display_graph("V")
                    print(x)
